...
// PERBAIKAN BUG DISINI: TAMBAHKAN 'go.' PADA 'Binding'
...
// =========================================================================
// LOGIKA AMAN & AKURAT (SUDAH DI-FIX AGAR TIDAK MACET)
// =========================================================================
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            ...
            // Cek: Apakah pada 'endP', dia sudah punya minimal 5 Direct Downline?
            const directPast = globalData.filter(u => u.upline === mem.uid && new Date(u.joinDate) <= endP).length;

            // SYARAT FINAL:
            // - Growth >= Target (>0)
            // - Punya minimal 5 Direct Downline pada SAAT ITU (Indikasi VIP 1)
            if (grow >= target && grow > 0 && directPast >= 5) {
                const rankNow = getRankLevel(mem.uid); // Tampilkan label rank terkini
