...
// ============================================================
// LOGIKA PIALA (DIPERBAIKI: PERIODE LALU & STATUS HISTORIS)
// ============================================================
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            ...
            // Syarat VIP minimal 5 member (sesuai rank tier paling bawah VIP 1)
            const teamCountAtPeriodEnd = dls.filter(dl => new Date(dl.joinDate) <= endP).length;
            if (teamCountAtPeriodEnd < 5) return; 

            // 2. HITUNG GROWTH
            // Base = Member sebelum periode dimulai (+1 diri sendiri utk base calculation)
            const baseCount = dls.filter(dl => new Date(dl.joinDate) < startP).length;
            
            // Grow = Member yg join DI ANTARA startP dan endP
            ...
            // Contoh A (0 anggota): BaseCount 0 -> TotalBase 1 -> Target = Ceil(0.5) = 1.
            ...
            if (grow >= target && grow > 0) {
                // Tampilkan Rank saat ini untuk label
