...
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            ...
            // Cek apakah PADA AKHIR PERIODE ITU dia punya minimal 5 Direct Downline?
            // Ini adalah indikator ringan untuk memastikan dia sudah/hampir VIP 1 saat itu.
            const directPast = globalData.filter(u => u.upline === mem.uid && new Date(u.joinDate) <= endP).length;

            // SYARAT FINAL:
            // 1. Growth >= Target dan > 0
            // 2. Rank SEKARANG minimal VIP 1
            // 3. (PENTING) Direct Downline SAAT ITU minimal 5 -> Mencegah "New VIP" masuk daftar masa lalu
            if (grow >= target && grow > 0 && rankNow >= 1 && directPast >= 5) {
