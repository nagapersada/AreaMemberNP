    ...
    if (sorted.length > 0) {
        sorted.forEach(m => {
            ...
            body.innerHTML += \`<div class="v-item \${isNew ? 'new-name-alert' : ''}">
                <div style="display:flex; flex-direction:column;"><span class="v-n">\${m.name} \${isNew ? '' : ''}</span><small style="color:#666; font-size:9px;">\${dateStr}</small></div>
...
function openAchieverModal() {
    ...
    modal.style.display = 'flex'; body.innerHTML = '<div class="v-empty">Sedang menghitung data... (Mohon Tunggu)</div>'; btnDl.style.display = 'none'; achieverTxtContent = "";
    ...
    setTimeout(() => {
        ...
        // 1. BUAT SNAPSHOT DATA MASA LALU (Supaya Cepat & Tidak Recursive Berulang)
        const pastMembers = globalData.filter(m => new Date(m.joinDate) <= endP);
        ...
        myTeamData.forEach(mem => {
            ...
            if (grow >= target && grow > 0) {
                // CEK PANGKAT PADA SAAT ITU (MENGGUNAKAN SNAPSHOT)
                const rankAtTime = getHistoricalRank(mem.uid, pastMembers);
                if (rankAtTime >= 1) {
...
// FUNGSI KHUSUS CEK PANGKAT MASA LALU (TERPISAH DARI LOGIN)
function getHistoricalRank(uid, pastData) {
    ...
    function countVipPast(team, lvl) {
        ...
        for(let i=1; i<team.length; i++) {
            // Rekursif aman karena pakai data snapshot yang sudah mengecil
            if(getHistoricalRank(team[i].uid, pastData) >= lvl) c++;
        }
        return c;
    }
