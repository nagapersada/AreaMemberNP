...
// RANKING LOGIC (STANDARD - DIPAKAI DASHBOARD)
...
// ============================================
// FIX PENTING: LOGIKA BERAT DIISOLASI DISINI
// ============================================
function openAchieverModal() {
    ...
    // Gunakan Timeout agar UI tidak Freeze
    setTimeout(() => {
        ...
        // SNAPSHOT DATA MASA LALU (Supaya Cepat & Aman)
        ...
        myTeamData.forEach(mem => {
            ...
            if (grow >= target && grow > 0) {
                // HANYA HITUNG RANK DISINI, JANGAN DI LOGIN
                const rankAtTime = getHistoricalRankSafe(mem.uid, pastData);
                if (rankAtTime >= 1) {
...
// FUNGSI HITUNG PANGKAT MASA LALU (TERPISAH & AMAN)
function getHistoricalRankSafe(uid, pastSnapshot) {
    function getDownlinesPast(u) {
        let list = [];
        pastSnapshot.filter(m => m.upline === u).forEach(d => { list.push(d); list = list.concat(getDownlinesPast(d.uid)); });
        return list;
    }
    function countVipPast(team, lvl) {
        let c = 0;
        for(let i=1; i<team.length; i++) {
            if(getHistoricalRankSafe(team[i].uid, pastSnapshot) >= lvl) c++;
        }
        return c;
    }

    const member = pastSnapshot.find(m => m.uid === uid);
    if(!member) return 0;
    
    const team = [member, ...getDownlinesPast(uid)];
    const directs = pastSnapshot.filter(m => m.upline === uid).length;
