...
// ============================================================
// STRUKTUR UTAMA (KEMBALI KE ASAL AGAR LOGIN LANCAR)
// ============================================================
...
function renderDashboard(){
    ...
    // DASHBOARD: MENGGUNAKAN CEIL AGAR KONSISTEN DENGAN MODAL
...
// ============================================================
// LOGIKA PIALA (DIPERBAIKI: PERIODE LALU & STATUS AKHIR PERIODE)
// ============================================================
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        if (d > 15) { 
            // Skrg Tgl 16-31 -> Lihat Periode 1 (Tgl 1-15 bln ini)
            startP = new Date(y, m, 1); 
            endP = new Date(y, m, 15, 23, 59, 59); 
            plabel = `PERIODE 1 (${getMonthName(m)} ${y})`; 
        } 
        else { 
            // Skrg Tgl 1-15 -> Lihat Periode 2 (Tgl 16-Akhir bln lalu)
            let pm = m - 1, py = y; 
            if (pm < 0) { pm = 11; py--; } 
        ...
        myTeamData.forEach(mem => {
            // Abaikan yg join SETELAH periode selesai
            if (new Date(mem.joinDate) > endP) return;

            const dls = getDownlinesRecursive(mem.uid);
            
            // 1. CEK STATUS VIP PADA SAAT AKHIR PERIODE TERSEBUT (HISTORIS)
            // Hitung total tim dia TEPAT pada tanggal 'endP'
            const teamCountAtPeriodEnd = dls.filter(dl => new Date(dl.joinDate) <= endP).length;
            
            // Syarat: Di akhir periode tsb, dia harus punya min 5 member (Status VIP)
            // Ini menangkap kasus: Awal 0 -> Ajak 5 -> Akhir Periode punya 5 -> Lolos jadi VIP
            if (teamCountAtPeriodEnd < 5) return; 

            // 2. HITUNG GROWTH
            // Base = Member sebelum periode dimulai
            const baseCount = dls.filter(dl => new Date(dl.joinDate) < startP).length;
            
            // Growth = Member yg join DI ANTARA startP dan endP
            const grow = dls.filter(dl => {
                const jd = new Date(dl.joinDate);
                return jd >= startP && jd <= endP;
            }).length;
            
            // 3. HITUNG TARGET (MATH.CEIL - Pembulatan ke Atas)
            // Untuk "0 anggota", Base dianggap 1 (diri sendiri) agar Target = ceil(1/2) = 1
            // Rumus: (Base + 1) / 2 dibulatkan ke atas.
            // Contoh A (0 anggota): BaseCount 0 -> TotalBase 1 -> Target 1.
            const totalBaseForTarget = baseCount + 1;
            const target = Math.ceil(totalBaseForTarget / 2);
            
            // SYARAT LOLOS: Growth >= Target DAN ada pertumbuhan minimal 1
            if (grow >= target && grow > 0) {
                const currentRank = getRankLevel(mem.uid); // Rank skrg utk display
