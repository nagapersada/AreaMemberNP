...
// LOGIKA SORTING VIP (YANG SUDAH BENAR)
...
// Syarat:
// 1. Growth >= Target (50% dari Base)
// 2. Growth > 0 (Harus ada pertumbuhan)
// 3. DI PERIODE LALU SUDAH VIP (Cek: Referral Direct >= 5 saat periode berakhir)
// =========================================================================
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            // Filter 1: Hapus member yang bergabung SETELAH periode ini berakhir
            if (new Date(mem.joinDate) > endP) return;

            const dls = getDownlinesRecursive(mem.uid);
            
            // Base: Jumlah tim SEBELUM periode dimulai
            const base = dls.filter(dl => new Date(dl.joinDate) < startP).length + 1;
            
            // Growth: Jumlah tim yang bergabung SELAMA periode berlangsung
            ...
            // Rank Saat Ini (Untuk label tampilan saja)
            const currentRank = getRankLevel(mem.uid);
            
            // LOGIKA KUNCI: CEK STATUS VIP DI MASA LALU (SAAT END PERIODE)
            // Hitung berapa referral langsung yang dimiliki TEPAT SAAT periode berakhir.
            ...
            // SYARAT LOLOS:
            // 1. Growth >= Target (Tumbuh 50%)
            // 2. Growth > 0 (Wajib ada member baru)
            // 3. directDownlinesAtEndP >= 5 (Saat periode berakhir, dia SUDAH jadi VIP/Punya 5 Direct)
