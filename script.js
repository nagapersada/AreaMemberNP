...
async function loadData(){
    try{
        ...
        // 2. Ambil Data History (TAMBAHAN UTK SORTING VIP)
...
function renderDashboard(){
    ...
    const g=[b,...d];
    myTeamData = g; 
    countVipStats(g); // Hitung VIP & Save History
...
// LOGIKA PANGKAT ASLI (TIDAK DIUBAH SAMA SEKALI)
...
// === BAGIAN BARU: FUNGSI SIMPAN HISTORY ===
async function checkAndSaveHistory(uid, level) {
    ...
    if (!exists) {
        ...
        db.from('vip_history').insert([{ uid: uid, vip_level: level, achieved_at: now }])
          .then(({ error }) => { if (error) console.log("History save error:", error); });
    }
}
...
function countVipStats(a){
    ...
    a.forEach(m=>{
        ...
        if(c>=1&&c<=9){
            ...
            // TAMBAHAN: AUTO SAVE HISTORY
            checkAndSaveHistory(m.uid, c);
            
            // TAMBAHAN: CEK ALERT BERDASARKAN HISTORY
...
// === MODAL VIP: EDIT SORTING (TERBARU -> TERLAMA) ===
window.openVipModal = function(level) {
    ...
    // SORTING: Dari yang paling baru tercatat di history
...
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(mem => {
            if (new Date(mem.joinDate) > endP) return; // Abaikan yg join setelah periode berakhir
            ...
            // VALIDASI: Cek referral langsung saat itu (Minimal 5)
            const directAtTime = globalData.filter(b => b.upline === mem.uid && new Date(b.joinDate) <= endP).length;

            // SYARAT YANG ANDA MINTA:
            // 1. Growth harus >= target
            // 2. Growth > 0 (Harus ada pertumbuhan)
            // 3. Rank >= 1 (VIP)
            // 4. Direct >= 5 (Sudah VIP saat periode itu)
